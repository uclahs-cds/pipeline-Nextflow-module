...
process convert_cram2bam_SAMtools {
    ...
    publishDir path: "${options.intermediate_output_dir}/${task.process.split(':')[-1].replace('_', '-')}_${sample_name}",
        enabled: options.save_intermediate_files,
        pattern: "*.bam",
        mode: 'copy'

    publishDir path: "${options.process_log_output_dir}/${task.process.split(':')[-1].replace('_', '-')}_${sample_name}",
        pattern: ".command.*",
        mode: "copy",
